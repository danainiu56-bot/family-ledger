...
Page({
  ...
  onLoad: function (options) {
    this._book = {
      id: '',
      data: L.emptyData(),
      current: L.cycleAnchor(new Date()),
      editing: null,
      payingId: null,
      bookForce: false
    };
  ...
  pullFromCloud: function (silent, force) {
    ...
    return L.cloudFetch(this._book.id).then(function (row) {
      ...
      that.setSync('err');
      that.setData({ syncFailed: true, syncErrorMsg: that.cloudErrMsg(err, '云端同步失败') });
      return false;
    });
  },
  cloudErrMsg: function (err, prefix) {
    var msg = (err && err.message) ? err.message : '';
    if (msg.indexOf('domain') !== -1 || msg.indexOf('not in domain list') !== -1) {
      return '真机/体验版需在小程序后台「开发设置 → request 合法域名」添加 ' +
        L.supabaseHost() + '（仅域名，不带路径）。';
    }
    if (msg.indexOf('ledgers') !== -1 || msg.indexOf('PGRST') !== -1) {
      return '云端表 ledgers 未创建，家人无法看到数据。';
    }
    return prefix + '：' + (msg || '网络异常');
  },
  save: function () {
    var that = this;
    ...
    if (L.cloudEnabled() && this._book.id) {
      ...
      L.cloudPush(this._book.id, this._book.data).then(function () {
        ...
        that.setSync('err');
        that.setData({ syncFailed: true, syncErrorMsg: that.cloudErrMsg(err, '保存到云端失败') });
      });
    }
  },
  retrySync: function () {
  ...
  joinBook: function (id, isNew) {
    ...
    return this.pullFromCloud(false, true).then(function () {
      that.recompute();
      that.startPoll();
      if (that.data.syncFailed) {
        wx.showToast({ title: '加入失败，请看顶部红色提示', icon: 'none' });
        return false;
      }
  ...
  onInput: function (e) {
    ...
    if (k === 'payNote') { this.setData({ 'pay.note': v }); return; }
  ...
  openPayment: function (e) {
    ...
    this.setData({
      ...
      pay: { amount: '', note: '', date: L.todayISO() }
    });
  },
  ...
  recompute: function () {
    ...
    var expenseRows = L.sortExpensesForDisplay(m.expenses).map(function (it) {
      ...
      var row = {
        ...
        notesText: L.paymentNotesSummary(it),
    ...
    this.setData({
      monthKey: key,
      monthRange: L.cycleRangeText(b.current),
